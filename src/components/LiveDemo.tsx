...
const analysisResults = {
  ...
  riskScore: 23,
  keyFindings: ["Unlimited liability clause in Section 8.3 (HIGH RISK)", "Automatic renewal terms in Section 4.2 (MEDIUM RISK)", "IP indemnification coverage adequate (LOW RISK)", "Termination clauses favor licensee (LOW RISK)"],
  recommendedActions: ["Negotiate liability cap in Section 8.3", "Add termination notice period", "Review data processing terms"]
};
const LiveDemo = () => {
  ...
  return <section className="w-full py-32 bg-background">
        ...
        <div className="grid lg:grid-cols-2 gap-20">
          ...
          <div className="premium-card fade-in-up">
            ...
            <div className="space-y-6 mb-10">
              ...
              return <div key={step.id} className="flex items-center space-x-4">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 transition-all duration-500 ${isCompleted ? 'bg-accent border-accent' : isActive ? 'border-accent bg-accent/10' : 'border-muted'}`}>
                      <Icon className={`w-6 h-6 ${isCompleted ? 'text-white' : isActive ? 'text-accent' : 'text-muted-foreground'}`} />
                    </div>
                    <div>
            ...
            <div className="border-2 border-dashed border-muted rounded-lg p-12 text-center mb-8">
              ...
              {currentStep === 1 && <Button onClick={startDemo} className="btn-accent">
                  Start Analysis Demo
                </Button>}
              
              {currentStep === 2 && <div className="flex items-center justify-center space-x-2">
                  <Clock className="w-5 h-5 animate-spin text-accent" />
                  <span className="text-accent font-semibold">Processing...</span>
                </div>}
              
              {currentStep === 3 && <div className="flex items-center justify-center space-x-2 text-accent">
          ...
          <div className={`space-y-8 transition-all duration-1000 ${showResults ? 'opacity-100' : 'opacity-30'}`}>
            {/* Risk Score */}
            <div className="premium-card fade-in-up stagger-2 bg-transparent">
              ...
              <div className="w-full bg-muted rounded-full h-3">
                <div className="bg-accent h-3 rounded-full transition-all duration-1000" style={{
                width: `${analysisResults.riskScore}%`
              }}></div>
              </div>
            </div>
            ...
            <div className="premium-card fade-in-up stagger-4">
              ...
              <div className="space-y-3">
                {analysisResults.keyFindings.map((finding, index) => <div key={index} className="flex items-start space-x-3">
                    <div className={`w-2 h-2 rounded-full mt-2 ${finding.includes('HIGH') ? 'bg-red-500' : finding.includes('MEDIUM') ? 'bg-yellow-500' : 'bg-green-500'}`}></div>
            ...
            <div className="premium-card fade-in-up stagger-5">
              ...
              <div className="space-y-2">
                {analysisResults.recommendedActions.map((action, index) => <div key={index} className="flex items-center space-x-3">
